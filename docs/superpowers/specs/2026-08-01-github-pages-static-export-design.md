...
4. Node.js 构建后脚本将 percent-encoded 文件名还原为 Unicode，确保 GitHub Pages 能解析中文指南 URL。
5. 静态导出写入 `dist/client`；Pages 工作流只上传这一目录，不上传 Worker 服务端产物。
6. GitHub Pages 将该构建产物发布到 `https://psiqaq.github.io/`。
...
- `scripts/decode-static-paths.mjs` 仅规范化导出文件名，并在路径不安全或目标冲突时让构建失败。
...
- 检查 `dist/client/index.html`、`dist/client/404.html`、中文指南的 Unicode `.html` 文件和静态资源目录。
